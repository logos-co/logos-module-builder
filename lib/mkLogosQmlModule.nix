...
let
  ...
  packages = forAllSystems (system:
    ...
    in {
      ...
      #   $out/lib/   — QML files and icons (bundler looks for lib/<main>)
      #   $out/share/ — metadata.json (bundler reads from drv.src/metadata.json at eval time)
      lib = (pkgs.runCommand "logos-${config.name}-lib" {} ''
        ...
        cp ${configFile} $out/share/metadata.json
      '') // { inherit src; };
    }
  );
