...
This document describes all available fields in `metadata.json` — the single configuration file for a Logos module. It is read by both the Qt runtime (top-level fields) and the Nix build system (fields under the `"nix"` key).
